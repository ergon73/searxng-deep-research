...
_NEWS_PATTERNS = [
    r"\bnews\b", r"\bновост[ьи]\b", r"сегодня", r"вчера", r"latest",
    r"breaking", r"событи[ея]", r"сообщил[аи]?",
    ...
    r"\bвышел\b", r"\bвышла\b", r"\bвыпустил[аи]?\b",  # "вышел", "выпустил"
    r"\bзапущен[аы]?\b", r"\bзапустил[аи]?\b", r"\bрелиз[а-яё]*\b",
    r"\bанонсиров[а-яё]*\b", r"\bпредстав[а-яё]*\b",  # "запущен", "релиз", "анонсирован"
    r"\bcurrent\b", r"\bтекущ[аио]?\b",
]

_FORUMS_PATTERNS = [
    r"\breddit\b", r"\bфорум[ауом]?\b", r"\bforum[s]?\b", r"\bdiscussion\b",
    r"\bdiscourse\b", r"\bсообществ[ао]\b", r"hacker ?news", r"\bhn\b",
    r"\bобсуждени[ея]\b",
]

_REVIEWS_PATTERNS = [
    r"\bотзыв[аыу]?\b", r"\breview[s]?\b", r"\buser experience\b",
    r"\bопыт\b", r"мнени[ея] польз[ователей]*", r"\bopinions?\b",
    r"что дума[ею]т[ь]? о", r"стоит ли покупать",
]

_DOCS_PATTERNS = [
    r"\bдокументаци[яи]\b", r"\bdocs?\b", r"\bdocumentation\b",
    r"\bapi reference\b", r"\btutorial\b", r"\bгайд[ау]?\b",
    r"\bhow to\b", r"\bmanual\b", r"\bруководств[оа]\b",
    r"\bgetting started\b",
]

_ACADEMIC_PATTERNS = [
    r"\barxiv\b", r"\bresearch paper\b", r"\bисследовани[ея]\b",
    r"\bстать[яьи]\b", r"\bpaper[s]?\b", r"\bpubmed\b",
    r"\bsemantics? scholar\b", r"\bpeer[- ]reviewed\b",
    r"\bacademic\b", r"\bнаучн[аыо]\b", r"\bдиссертаци[яи]\b",
    r"\bstudy\b",  # weak signal: requires context
]

_GITHUB_PATTERNS = [
    r"\bgithub\b", r"\brepo(sitory)?\b", r"\bрепозитори[йя]\b",
    r"\bsource code\b", r"\bисходник[иа]?\b", r"\bgit clone\b",
]
...
_TECHNICAL_PATTERNS = [
    ...
    r"\bspacex\b", r"\bfalcon\s*9\b", r"\bfalcon\s*heavy\b",
    r"\bnasa\b", r"\broscosmos\b", r"\bESA\b",
    r"\bapple\b", r"\biphone\b", r"\bmacbook\b", r"\bipad\b",
    r"\bnvidia\b", r"\btesla\b", r"\bamd\b", r"\bintel\b",
    r"\bsamsung\b", r"\bgoogle\b", r"\bmicrosoft\b",
    # AI models
    r"\bgpt[- ]?\d?\b", r"\bchatgpt\b", r"\bclaude\b", r"\bgemini\b",
    r"\bllama\b", r"\bdeepseek\b", r"\bmistral\b",
    # Technical nouns (RU + EN)
    r"\bракет[аыу]?\b", r"\bспутник[иа]?\b", r"\bстарт[аы]?\b",
    r"\bзапус[кт][а-яё]*\b", r"\bступен[ьи]\w*\b",
    r"\brocket[s]?\b", r"\blaunch(es)?\b", r"\bsatellite[s]?\b",
    # Technical metrics
    r"\btda?\b", r"\bgpu\b", r"\bcpu\b", r"\bram\b", r"\bssd\b",
    r"\bprocessor\b", r"\bчип[ау]?\b", r"\bвидеокарт[аыу]?\b",
    r"\bпроцессор[ау]?\b", r"\bоперативн[аяо][\s-]?памят[ьи]\b",
]
...
_WIKI_PATTERNS = [
    ...
    r"\bчто такое\b", r"\bчто\s+значит\b", r"\bопределени[ея]\b",
    r"\bwhat is\b", r"\bwhat are\b", r"\bdefine\b", r"\bdefinition\b",
    # Founding / history (RU)
    r"\bоснован[аы]?\b", r"\bосновани[ея]\b", r"\bобразован[аы]?\b",
    r"\bсоздан[аы]?\b", r"\bучрежд[ёе]н[аы]?\b",
    r"\bfounded\b", r"\bestablished\b", r"\bformation\b",
    # Countries / geography / people
    r"\bстран[аыуо]?\s*[-\s]?член[аов]?\b",
    r"\bстран[аыуо]?\b", r"\bгород[ау]?\b", r"\bнаселённ[аыо]?\w*\b",
    r"\bстолиц[аыу]?\b", r"\bнаселение\b", r"\bтерритори[яи]\b",
    r"\bcountry\b", r"\bcity\b", r"\bpopulation\b", r"\bcapital\b",
    # Scientific facts
    r"\bтемператур[аыу]?\s+плавлени[яе]\b",
    r"\bмасса\b", r"\bатомн[аыо]\s+масс[аыу]?\b",
    r"\bмолекул[аыу]?\b", r"\bэлемент[аы]?\b",
    r"\bвселенн[аыу]?\b", r"\bпланет[аыу]?\b", r"\bзвезд[аыу]?\b",
    r"\bматери[яи]\b", r"\bэнерги[яи]\b", r"\bквантов[аыо]?\b",
    r"\bgalaxy\b", r"\buniverse\b", r"\bplanet[s]?\b", r"\bstar[s]?\b",
    r"\bmatter\b", r"\benergy\b", r"\bquantum\b",
]

_SECURITY_PATTERNS = [
    r"\bcve\b", r"\bvulnerab(ilit)?y\b", r"\bexploit\b",
    r"\b0[- ]?day\b", r"\bsecurity advisory\b", r"\bpatch(ed)?\b",
    r"\bуязвимост[ьи]\b", r"\bэксплойт[ау]?\b",
]

_PRODUCT_PATTERNS = [
    r"\bvs\.?\b", r"\bcompared?\b", r"\bсравн[еи]ть?\b",
    r"\balternative[s]?\b", r"\bаналог[иа]?\b", r"\bобзор[аы]?\b",
    r"\bpricing\b", r"\bchangelog\b", r"\bbenchmark[s]?\b",
]
...
_RECENCY_PATTERNS = [
    r"\blatest\b", r"\bновейш\b", r"\bпоследн[иея]?\b", r"\bнедавн[ио]?\b",
    r"\bcurrent\b", r"\bтекущ[аио]?\b", r"\b2026\b", r"\b2025\b",
    r"\bв этом году\b", r"\byesterday\b", r"\bвчера\b",
    r"\btoday\b", r"\bсегодня\b",
]

_RECENCY_TIME_RANGES = {
    # Words that suggest very recent (last day)
    "day": [r"\btoday\b", r"\bсегодня\b", r"\bвчера\b", r"\byesterday\b",
            r"\bbreaking\b", r"\bтолько что\b"],
    # Last week
